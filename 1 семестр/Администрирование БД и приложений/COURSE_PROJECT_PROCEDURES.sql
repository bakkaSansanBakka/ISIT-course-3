...
(p_company_name suppliers.?ompany_name%type,
p_adress suppliers.adress%type,
p_phone_number suppliers.phone_number%type)
is
begin
    insert into SUPPLIERS(?ompany_name, adress, phone_number) 
...
(p_company_name manufacturers.?ompany_name%type,
p_adress manufacturers.adress%type,
p_phone_number manufacturers.phone_number%type)
is
begin
    insert into MANUFACTURERS(?ompany_name, adress, phone_number) 
    VALUES(p_company_name, p_adress, p_phone_number);
    exception
    when others 
    then DBMS_OUTPUT.put_line(sqlerrm);
...
xmlelement("?ompany_name", SUPPLIERS.?ompany_name),
...
Dbms_Xslprocessor.Valueof(V_Current_Node,'?ompany_name/text()',et.?ompany_name);
...
insert into SUPPLIERS(?ompany_name, adress, phone_number)
values(et.?ompany_name, et.adress, et.phone_number);
end loop;
