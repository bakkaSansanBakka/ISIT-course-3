...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		if (!SetNamedPipeHandleState(cH, &dwMode, NULL, NULL))
			throw SetPipeError("Set Named Pipe Handle State: ", GetLastError());
		...
		for (int i = 1; i < numberOfMessages + 1; i++)
		{
			...
			if (!TransactNamedPipe(cH, wbuf, sizeof(wbuf), rbuf, sizeof(rbuf), &lrbuf, NULL))
				throw SetPipeError("Transact: ", GetLastError());
			
			cout << "Message " << i << " delivered" << endl;
		}
