...
char serverName[50] = "LAPTOP-JA53F0CF";
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        int gethostnameCodeResult;
        
        if ((gethostnameCodeResult = gethostname(serverName, sizeof(serverName))) == SOCKET_ERROR)
            throw SetErrorMsgText("gethostname:", WSAGetLastError());
        else cout << "Server name: " << serverName << endl;


        if (GetServer(servName, serverAnother.sin_port, (sockaddr*)&serverAnother, &len)) {
            cout << endl << "Found another Server (" << inet_ntoa(serverAnother.sin_addr) << ":" << htons(serverAnother.sin_port) << ")";
        }
        else {
        ...
        while (true) {
            if (GetRequestFromClient(servName, serv.sin_port, (sockaddr*)&clnt, &lc)) {
                ...
                clntInfo = gethostbyaddr((const char*)&serv.sin_addr, 4, AF_INET);
