...
# LogisticRegression и LinearSVC для набора forge
...
# исследование логистической регрессии на примере набора
# с раком женской груди
...
# увеличим С чтобы построить более гибкую модель
...
# уменьшим С для построения более регуляризованной модели
...
# коэффициенты логистической регрессии с тремя различными
# параметрами регуляризации
...
# использование L1 регуляризации для построения
# более интерпретабельной модели
for C, marker in zip([0.001, 1, 100], ['o', '^', 'v']):
    lr_l1 = LogisticRegression(C=C, penalty="l2").fit(X_train, y_train)
    print("Правильность на обучающем наборе для логрегрессии l1 с C={:.3f}: {:.2f}".format( C, lr_l1.score(X_train, y_train)))
    print("Правильность на тестовом наборе для логрегрессии l1 с C={:.3f}: {:.2f}".format( C, lr_l1.score(X_test, y_test)))
