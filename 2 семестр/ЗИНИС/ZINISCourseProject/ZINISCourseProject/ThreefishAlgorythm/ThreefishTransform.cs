...
namespace ZINISCourseProject
{
    ...
    public class ThreefishTransform : ICryptoTransform
    {
        ...
        // ECB mode decryption
        public int EcbDecrypt(byte[] input, int inputOffset, int inputCount, byte[] output, int outputOffset)
        {
            if (inputCount < cipherBytes)
                return 0;
            GetBytes(input, inputOffset, block, cipherBytes);
        ...
        // CBC mode encryption
        public int CbcEncrypt(byte[] input, int inputOffset, int inputCount, byte[] output, int outputOffset)
        {
            if (inputCount < cipherBytes)
                return 0;
            GetBytes(input, inputOffset, block, cipherBytes);
        ...
        // CBC mode encryption
        public int CbcDecrypt(byte[] input, int inputOffset, int inputCount, byte[] output, int outputOffset)
        {
            if (inputCount < cipherBytes)
                return 0;
            GetBytes(input, inputOffset, block, cipherBytes);
        ...
        public int OfbApplyStream(byte[] input, int inputOffset, int inputCount, byte[] output, int outputOffset)
        {
        ...
        // CFB mode encryption
        public int CfbEncrypt(byte[] input, int inputOffset, int inputCount, byte[] output, int outputOffset)
        {
            for (int i = 0; i < inputCount; i++)
            {
                // Generate new stream bytes if we've used
        ...
        // CFB mode decryption
        public int CfbDecrypt(byte[] input, int inputOffset, int inputCount, byte[] output, int outputOffset)
        {
            for (int i = 0; i < inputCount; i++)
            {
                // Generate new stream bytes if we've used
